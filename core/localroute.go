...
func LoadLocalRouteConfig(nodeId int, path string) (*LocalRouteConfig, error) {
	if path == "" {
		return nil, nil
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			log.Warnf("[Node %d] Local route config file not found: %s", nodeId, path)
			return nil, nil
		}
	...
	log.Warnf("[Node %d] Loaded local route config from %s (outbounds: %d, rules: %d)", nodeId, path, outboundCount, ruleCount)
	return config, nil
}

// BuildLocalOutbounds 构建本地outbound配置
func BuildLocalOutbounds(nodeId int, config *LocalRouteConfig) ([]*core.OutboundHandlerConfig, error) {
	...
	for i := range config.Outbounds {
		...
		if err != nil {
			log.Warnf("[Node %d] Build local outbound %d error: %v", nodeId, i, err)
			continue
		}
